...
public:
    ...
    FaceRosyField(const Hmsh& mesh, const int nRosy): FaceTangentField(mesh, nRosy) {}
    FaceRosyField(const Hmsh& mesh, const int nRosy, FieldType type): FaceTangentField(mesh, nRosy) {
    ...
    void computeConnection() {
        ...
        for (auto e: mesh.edges | std::views::filter([](auto _e) { return !_e.isBoundary(); })) {
            ...
            complex ef(v.dot(e.f0().basisX()), v.dot(e.f0().basisY()));
            complex eg(v.dot(e.f1().basisX()), v.dot(e.f1().basisY()));
...
private:
    void pMatching(VecXd& effort) {
        for (auto e: mesh.edges | std::views::filter([](auto e) { return !e.isBoundary(); })) {
            auto conn = connection[e.id];
            auto minRot = 1000.;
            auto offset = 0;
            complex coef(1, 0);
            complex transport0 = conn * field(e.f0().id, 0);
            for (int i = 0; i < rosyN; i++) {
                complex jf = field(e.f0().id, i);
                complex jg = field(e.f1().id, i);
                coef *= jg / (conn * jf);
            ...
            double residuals = 0;
            for (int i = 0; i < rosyN; i++) {
                auto v0 = field(e.f0().id, i);
                auto v1 = field(e.f1().id, (i + offset) % rosyN);
    ...
    void cMatching(VecXd& effort) {
        for (auto e: mesh.edges | std::views::filter([](auto e) { return !e.isBoundary(); })) {
            ...
            for (int i = 0; i < rosyN; i++) {
                auto v0 = field(e.f0().id, i);
                auto v1 = field(e.f1().id, (i + iM) % rosyN);
